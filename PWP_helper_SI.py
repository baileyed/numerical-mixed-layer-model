...
def set_params(lat, dt=1., dz=1.0, max_depth=150., mld_thresh1=1e-1, dt_save=1., rb=0.65, rg=0.25, rkz=1e-6, beta1=0.6, beta2=20.0, heat_ON=True, winds_ON=True, emp_ON=False, drag_ON=True, ice_ON=True):
    ...
    dt: time-step increment. Input value in units of hours, but this is immediately converted to seconds.[1 hour]
    ...
    params['mld_thresh1'] = mld_thresh1
    params['mld_thresh2'] = 1e-2
    
    #added for the sea-ice component
    params['rhoi'] = 900
    params['lati'] = 3.34e5 #J/K
    params['ch'] = 0.006
    params['rhow'] = 1000
    params['h_i'] = 1.7
...
def prep_data(met_dset, prof_dset, params):
    ...
    if params['ice_ON'] == False:
        print("WARNING: Sea Ice is turned OFF.")
        forcing['ice'][:] = 0.0
    
    
    #define q_in and q_out (positive values should mean ocean warming)
    forcing['q_s'] = forcing['sw'] #heat flux into ocean
...
#             p_intp = interp1d(prof_dset['z'], prof_dset[vname], axis=0, kind='linear', bounds_error=False)
    ...
    pwp_out['temp_lead'] = np.zeros(arr_sz)
    pwp_out['temp_afterice'] = np.zeros(arr_sz)
    pwp_out['sal'] = np.zeros(arr_sz)
    pwp_out['sal_afterice'] = np.zeros(arr_sz)
    pwp_out['dens'] = np.zeros(arr_sz)
    pwp_out['uvel'] = np.zeros(arr_sz)
    pwp_out['vvel'] = np.zeros(arr_sz)
    pwp_out['mld1'] = np.zeros((tlen,))
    pwp_out['mld2'] = np.zeros((tlen,))
    pwp_out['hi'] = np.zeros((tlen,))
    pwp_out['hi2'] = np.zeros((tlen,))
    pwp_out['ocnhf'] = np.zeros((tlen,))
    pwp_out['temp_surf'] = np.zeros((tlen,))
    pwp_out['temp_fz'] = np.zeros((tlen,))
    pwp_out['qnet_o'] = np.zeros((tlen,))
    pwp_out['qnet_i'] = np.zeros((tlen,))
    pwp_out['temp_mld1'] = np.zeros((tlen,))
    pwp_out['temp_mld2'] = np.zeros((tlen,))
    pwp_out['mld_idx2'] = np.zeros((tlen,))
    pwp_out['mld_idx1'] = np.zeros((tlen,))
    pwp_out['tau'] = np.zeros((tlen,))
    pwp_out['ice_area'] = np.zeros((tlen,))
    
    #use temp, sal and dens profile data for the first time step
    pwp_out['sal'][:,0] = sal0
    pwp_out['temp'][:,0] = temp0
    pwp_out['dens'][:,0] = dens0
    pwp_out['hi'][0] = params['h_i']
...
def makeSomePlots(forcing, pwp_out, time_vec=None, save_plots=False, suffix=''):
    ...
    axes[0].plot(tvec, forcing['q_s']-forcing['q_out'], ls='-', lw=2, color='k', label='$Q_{net}$')   
    ...
    ##plot wind stress
    axes[1].plot(tvec, forcing['tix'], label=r'$\tau_xi$')
    axes[1].plot(tvec, forcing['tiy'], label=r'$\tau_yi$')
    axes[1].plot(tvec, forcing['tax'], label=r'$\tau_xa$')
    axes[1].plot(tvec, forcing['tay'], label=r'$\tau_ya$')
